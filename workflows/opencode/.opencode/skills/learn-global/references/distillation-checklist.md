# Checklist de Distillation & Non-Pollution du Workflow (OpenCode)
...
| **Noms de Produits / Clients** | **INTERDIT** | Anonymiser ou generaliser les intitules. |
...
| **Regles de Gouvernance (.opencode/AGENTS.md)** | **AUTORISE** | Affiner les barrieres de synchronisation et les boucles iteratives. |
| **Scripts Utilitaires & Commandes Reutilisables** | **AUTORISE** | Integrer les commandes et scripts d'automatisation portables. |

---

## 2. Processus de Validation Avant Synchronisation

1. **Scan Automatique** : Executer `python3 .opencode/skills/learn-global/scripts/distill_workflow.py` en mode simulation (`dry-run`).
2. **Revue du Diff** : Verifier chaque ligne modifiee avec `git diff`.
3. **Verification de Generalisation** : Confirmer que la regle ou l'instruction amelioree s'applique a l'ensemble des futurs projets.
4. **Synchronisation Submodule** : S'assurer que le sous-module `docs/opencode` est propre et a jour.
5. **Commit Propre** : Enregistrer le commit dans le depot central avec une description conventionnelle explicite sans emoji.
